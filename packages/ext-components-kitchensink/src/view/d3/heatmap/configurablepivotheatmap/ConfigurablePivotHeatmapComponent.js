...
export default class ConfigurablePivotHeatmapComponent {
    ...
    mainCtnReady(event) {
        debugger;
        let mainCtn = event.detail.cmp;
        // mainCtn.setConfigurator(this.configuratorVar);
        mainCtn.setDrawing(this.drawingVar);
        mainCtn.setMatrix(this.matrixVar);

        console.log(mainCtn.getMatrix());
        // mainCtn.on('onBeforeMoveConfigField', this.onBeforeAddConfigField.bind(this))
        // mainCtn.on('onShowConfigFieldSettings', this.onShowFieldSettings.bind(this))
    }
}
