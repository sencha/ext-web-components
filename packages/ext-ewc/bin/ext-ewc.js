...
// A skeleton for a ext-ewc workspace.json file.
const workspaceJson = {
    apps: [],
...
printUsage = () => {
    console.log(
`
    ext-ewc CLI currently offers a method for generating a theme for ext-ewc apps (SASS based system).

    An example usage is:
        ext-ewc generate theme --baseTheme theme-material --name my-awesome-theme
...
/**
 * Ensures a 'ext-ewc/packages' folder exists for the workspace and theme packages to be installed in.
 */
const ensurePackagesFolder = () => {
    return new Promise(resolve => {
        const dir = path.join('.', 'ext-ewc', 'packages');
    ...
    return ensurePackagesFolder().then(() => {
        return new Promise((resolve, reject) => {
            if(!workspaceExists()) {
                ...
                fs.writeFile(path.join('.', 'ext-ewc', 'workspace.json'), JSON.stringify(workspaceJson, null, 4), err => {
                    if(err) return reject(err);
                    return resolve();
                });
            } else {
                console.log('Using existing workspace at ext-ewc/workspace.json');
                return resolve();
            }
        });
    });
...
const workspaceExists = () => {
    try {
        return fs.statSync(path.join('.', 'ext-ewc', 'workspace.json')).isFile();
    } catch(e) {
        return false;
    }
}
...
const generateTheme = config => {
    ...
    return new Promise((resolve, reject) => {
        const proc = fork(sencha, [
            ...
            '--framework', 'ext',
            '--name', config.name
        ], { cwd: path.join('.', 'ext-ewc'), silent: true });
...
const updatePackageJson = config => {
    const packageJsonPath = path.join('.', 'ext-ewc', 'packages', config.name, 'package.json');
...
const applyTheme = config => {
  ...
  return new Promise((resolve, reject) => {
      fs.writeFile('.ext-ewcsrc', JSON.stringify({theme: path.join('.', 'ext-ewc', 'packages', config.name)}, null, 4),
      err => {
          if(err) {
            return reject(err);
          }
...
switch(args._.join(' ')) {
    ...
    case 'generate theme': {
        ...
        return generateWorkspace(args)
            ...
            .then(() => {
              console.log(`Theme created at: ext-ewc/packages/${args.name}`);
