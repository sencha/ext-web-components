...
function (_HTMLElement) {
  _inheritsLoose(ExtBase, _HTMLElement);
  ...
  _proto.connectedCallback = function connectedCallback() {
    ...
    this.rawChildren = Array.from(this.children); //console.dir(`connectedCallback: ${this.nodeName} parent: ${this.nodeParentName} extParentDefined: ${this.extParentDefined} extChildrenDefined: ${this.extChildrenDefined} this.children.length: ${this.children.length}`)
    ...
    for (var property in this.PROPERTIESOBJECT) {
      if (this.PROPERTIESOBJECT.hasOwnProperty(property)) {
        if (this.getAttribute(property) !== null) {
          if (property == 'handler') {
            var functionString = this[property]; //error check for only 1 dot
        ...
        for (var i = this.parentNode.children.length - 1; i > -1; i--) {
          ...
          if (item.nodeName.substring(0, 3) == "EXT") {
            if (item.props == this.props) {
              ...
              this.parentNode.extChildren.push({
                ADDORDER: i,
                XTYPE: item.XTYPE,
                EXT: this.ext
              });
    ...
    for (var i = 0; i < this.extChildren.length; i++) {
      var item = this.extChildren[i]; //mjgComment console.log(`item ${i} ext child`)
  ...
  _proto.addTheChild = function addTheChild(parentCmp, childCmp, location) {
    ...
    if (parentxtype === 'grid' || parentxtype === 'lockedgrid') {
      if (childxtype === 'column' || childxtype === 'treecolumn' || childxtype === 'textcolumn' || childxtype === 'checkcolumn' || childxtype === 'datecolumn' || childxtype === 'rownumberer' || childxtype === 'numbercolumn' || childxtype === 'booleancolumn') {
        ...
        if (parentCmp.getHideHeaders() === false) {
          parentCmp.insert(1, childCmp); //          console.log('**')

          return;
        } else {
          parentCmp.add(childCmp); //          console.log('**')
    ...
    if (childxtype === 'tooltip') {
      parentCmp.setTooltip(childCmp); //      console.log('**')

      return;
    }

    if (childxtype === 'plugin') {
      parentCmp.setPlugin(childCmp); //      console.log('**')

      return;
    } else if (parentxtype === 'button') {
      if (childxtype === 'menu') {
        parentCmp.setMenu(childCmp); //        console.log('**')
    ...
    if (childxtype === 'toolbar' && Ext.isClassic === true) {
      parentCmp.addDockedItems(childCmp); //console.log('**')
      ...
      if (parentCmp.getHideHeaders() === false) {
        ...
        parentCmp.add(childCmp); //        console.log(`${parentCmp.xtype}.add(${childCmp.xtype})`)
    ...
    if (parentCmp.add != undefined) {
      if (location == null) {
        ...
        parentCmp.insert(location, childCmp); //mjgComment console.log(`${parentCmp.xtype}.insert(${location}, ${childCmp.xtype})`)
