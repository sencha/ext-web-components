...
//Tue Sep 10 2019 12:15:34 GMT-0400 (Eastern Daylight Time)
...
function (_HTMLElement) {
  ...
  function EwcBaseComponent() {
    ...
    _this = _HTMLElement.call(this) || this; //this.newDiv = document.createElement('div');
    //this.insertAdjacentElement('beforebegin', this.newDiv);

    _this.xtype = _this.XTYPE;
    _this.base = EwcBaseComponent; //this.XTYPE = .XTYPE()
  ...
  _proto.connectedCallback = function connectedCallback() {
    ...
    this.insertAdjacentElement('beforebegin', this.newDiv); //this.xtype = this.XTYPE;
    //this.base = EwcBaseComponent;

    this.properties = []; //console.log(this.PROPERTIESOBJECT)
  ...
  _proto.setHasParent = function setHasParent() {
    var hasParent;
  ...
  _proto.figureOutA = function figureOutA() {
    ...
    if (this.hasParent && this.parentEl.A == undefined && this.parentEl.nodeName.substring(0, 4) == 'EXT-') {
  ...
  _proto.createExtComponent = function createExtComponent() {
    ...
    if (A.props['viewport'] == true) {
      ...
      Ext.onReady(function () {
        ...
        methis.assessChildren(methis.base, methis.xtype);
        console.log('after assessChildren'); //var wc = meA.ext.el.dom.nextSibling;
  ...
  _proto.assessChildren = function assessChildren(base, xtype) {
    ...
    if (this._extitems != undefined) {
      if (this._extroutes.length == 1) {
        ...
        A.ext.setHtml(this._extroute.nativeElement); //childItem.childCmp = Ext.create({xtype:'widget', ewc:item.getAttribute('ewc'), element:Ext.get(item.parentNode.removeChild(item))})
      }
    }

    if (A.CHILDRENCOMPONENTSCOUNT == 0 && A.CHILDRENCOMPONENTSLEFT == 0 && A.CHILDRENCOMPONENTSADDED == 0 && !this.hasParent) {
      console.log('Solo');
      ...
      this.addChildren(this, A.CHILDRENCOMPONENTS); //console.log('send ready for CHILDRENCOMPONENTSADDED > 0');
  ...
  _proto.addChildren = function addChildren(child, children) {
    ...
    for (var i = 0; i < children.length; i++) {
      ...
      childItem.childCmp = A2.ext; //childItem.childCmp = children[i].currentEl.A.ext;
