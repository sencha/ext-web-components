...
export class Router {
  ...
  hasChanged(scope, routes) {
    if (window.location.hash.length > 0) {
      ...
      route.rootElem.style.display = "block"
      ...
      //route.rootElem.innerHTML = window._code[currentHashLower][componentHtml]
    }
    else {
      var currentHash = ''
      var currentHashLower = ''
...
export class EwcRouterComponent extends HTMLElement {
...
(function () {
    Ext.onReady(function() {
        window.customElements.define('ewc-router', EwcRouterComponent);
    });
})();