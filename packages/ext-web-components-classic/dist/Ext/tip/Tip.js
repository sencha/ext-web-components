...
function (_Ext_panel_Panel) {
  _inheritsLoose(Ext_tip_Tip, _Ext_panel_Panel);

  Ext_tip_Tip.PROPERTIES = function PROPERTIES() {
  ...
  Ext_tip_Tip.EVENTS = function EVENTS() {
    return [{
      name: 'activate',
      parameters: 'sender'
    }, {
  ...
  Ext_tip_Tip.getProperties = function getProperties(properties) {
    properties = properties.concat(Ext_tip_Tip.PROPERTIES());
    return Ext_panel_Panel.getProperties(properties);
  };

  Ext_tip_Tip.getEvents = function getEvents(events) {
    events = events.concat(Ext_tip_Tip.EVENTS());
    return Ext_panel_Panel.getEvents(events);
  } //events
  ////configs
  //
  //static XTYPE() {return 'tip'}
  ...
  _createClass(Ext_tip_Tip, null, [{
    key: "observedAttributes",
    get: function get() {
      var attrs = _Ext_panel_Panel.observedAttributes; //for (var property in Ext_tip_Tip.PROPERTIESOBJECT()) {
      //    attrs.push(property)
      //}

      Ext_tip_Tip.PROPERTIES().forEach(function (property, index, array) {
        attrs.push(property);
      });
      Ext_tip_Tip.EVENTS().forEach(function (eventparameter, index, array) {
  ...
  function Ext_tip_Tip(properties, events) {
    return _Ext_panel_Panel.call(this, properties.concat(Ext_tip_Tip.PROPERTIES()), events.concat(Ext_tip_Tip.EVENTS())) || this;
  }

  var _proto = Ext_tip_Tip.prototype;
