...
class VirtualRange extends Range {
    //---------------------------------------------------------------------------------
    // Life-cycle
    ...
    adjustPageLocks (kind, adjustment) {
        let me = this;
        let pages = me.adjustingPages;
        let n = pages.length;
        let i;
    ...
    gotoNow (begin, end) {
        let me = this;
        let prefetch = me.prefetch;
        let records = me.records;
        let store = me.store;
        let pageMap = store.pageMap;
        let limit = store.totalSize;
        let beginWas = me.lastBegin;
        let endWas = me.lastEnd;
        let activePagesWas = me.activePages;
        let prefetchPagesWas = me.prefetchPages;
        let beginBufferZone = me.trailingBufferZone;
        let endBufferZone = me.leadingBufferZone;
        let adjustingPages = me.adjustingPages;
        let activePages, page, pg, direction, prefetchBegin, prefetchEnd, prefetchPages;

        super.gotoNow(begin, end);  // goto() is @buffered so this is the original fn
    ...
    onPageDestroy (page) {
        let n = page.number;
        let activePages = this.activePages;
        let prefetchPages = this.prefetchPages;
    ...
    onPageLoad (page) {
        ...
        if (me.activePages[page.number]) {
            ...
            if (fn) {
                ...
                callback(fn, me.scope, [me, first, last]);
            }
        }
    }

    pageSortBackFn (page1, page2) {
        return page2.number - page1.number;
    }

    pageSortFwdFn (page1, page2) {
    ...
    reset () {
        this.records = {};
        this.activePages = this.prefetchPages = null;
    }
}
