 ...
 * in `Model`.**
 *
 * Associations enable you to express relationships between different {@link Model Models}.
 * Consider an e-commerce system where Users can place Orders - there is a one to many
 * relationship between these Models, one user can have many orders (including 0 orders).
 * This example will be referred to in the following sections.
 *
 *     Ext.define('User', {
 *         extend: 'Model',
 *         fields: [{
 ...
 * # Association Types
 *
 * Associations can describe relationships in 3 ways:
 *
 * ## Many To One
 *
 * A single entity (`A`) has a relationship with many (`B`) entities. An example of this is
 * an e-commerce system `User` can have many `Order` entities.
 *
 * This can be defined using `ManyToOne` for keyed associations, or 
 * `HasMany` for keyless associations.
 * 
 * ## One To One
 *
 ...
 *
 * This can be defined using `OneToOne` for keyed associations, or 
 * `HasOne` for keyless associations.
 * 
 * ## Many To Many
 *
 ...
 *
 * This can be defined using `ManyToMany`. Many To Many relationships are read-only
 * unless used with a `Session`.
 *     
 *
 ...
 *         console.log(orders1.size, orders1.first().getUser() === user1);
 *         // 2 orders, same reference back to user2
 *         console.log(orders2.size, orders2.first().getUser() === user2);
 *     });
 *
 * ## Binding
 *
 * Data binding using a {@link ViewModel} has functionality to recognize associated data
 * as part of a bind statement. For example:
 * 
 ...
 * follows a pattern defined by the `Namer`. To change a specific instance,
 ...
 * 
 * On records that have a "to many" relationship, a single methods that returns a `Store`
 ...
 * ## Naming
 *
 * Referring to model names in associations depends on their {@link Model#entityName}. See
 * the "Relative Naming" section in the `Schema` documentation.
 */
@define({
    tags: {
        isOneToOne: false,
...
class Association extends Base {
    setup () {
        ...
        me.left = left = new cls.Left(me.left);
        me.right = right = new cls.Right(me.right);

        left.association = right.association = me;
        left.inverse = right;
        right.inverse = left;
    }
     ...
     * @param {Store} [options.success.store] The store.
     * @param {Operation} [options.success.operation] The operation. `null`
     * if no load occurred.
     *
     ...
     * @param {Store} [options.failure.store] The store.
     ...
     * @param {Store} [options.callback.store] The store.
     * @param {Operation} [options.callback.operation] The operation. `null`
     * if no load occurred.
     ...
     * 
     * @param {Object} [scope] The `this` reference for the callback. Defaults to the record.
     * @return {Model} The record. `null` if the reference has been previously specified
     * as empty.
     */
    
    /**
     * @method recordSetter **This is not a real method, it is placeholder documentation
     * for a generated method on a `Model`.**
     *
     * Sets a model of the "one" type.
     * @param {Model/Object} value The value to set. This can be a record, a key value
     * (if a keyed association) or `null` to clear the value.
     *
     ...
     * are the same as {@link Model#save}. If  options is specified,
     * {@link Model#save} will be called on this record.
     */
